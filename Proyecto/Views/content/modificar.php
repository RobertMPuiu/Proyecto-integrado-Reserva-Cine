...
<head>
	...
	<link rel="stylesheet" type="text/css" href="../Views/styles/modificar.css">
			...
			<div class="row" id="recuadroPelicula">
					...
					*/
					foreach ($mostrar as $key => $value) {
	                  ?>
				<div class="offset-lg-1 offset-xl-3 col-xs-6 col-md-8 col-lg-6 col-xl-6" id="modificarPelicula">
	              <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
	                  <h2><?php echo $mostrar[$key]->getTitulo(); ?></h2>
	                  <input type="hidden" name="idPelicula" id="idPelicula" class="form-control" value="<?php echo $mostrar[$key]->getIdPelicula(); ?>">   	
	                  <div class="form-group">
	                    <label for="imagen">Portada</label>
	                    <input type="text" name="imagen" id="imagen" class="form-control" value="<?php echo $mostrar[$key]->getPortada(); ?>">
	                  </div>
	                  <div class="form-group">
	                    <label for="tituloPelicula">Titulo</label>
	                    <input type="text" name="tituloPelicula" id="tituloPelicula" class="form-control" value="<?php echo $mostrar[$key]->getTitulo(); ?>">
	                  </div>
	                   <div class="form-group">
	                    <label for="pais">Pais</label>
	                    <input type="text" name="pais" id="pais" class="form-control"  value="<?php echo $mostrar[$key]->getPais(); ?>">
	                  </div>
	                  <div class="form-group">
	                    <label for="genero">Género</label>
	                    <input type="text" name="genero" id="genero" class="form-control"  value="<?php echo $mostrar[$key]->getGenero(); ?>">
	                  </div>
	                  <div class="form-group">
	                    <label for="duracion">Duración</label>
	                    <input type="text" name="duracion" id="duracion" class="form-control"  value="<?php echo $mostrar[$key]->getDuracion(); ?>">
	                  </div>
	                  <div class="form-group">
	                    <label for="calificacion">Calificación</label>
	                    <input type="text" name="calificacion" id="calificacion" class="form-control"  value="<?php echo $mostrar[$key]->getCalificacion(); ?>">
	                  </div>
	                  <div class="form-group">
	                    <label for="fechaEstreno">Fecha de estreno</label>
	                    <input type="text" name="fechaEstreno" id="fechaEstreno" class="form-control"  value="<?php echo $mostrar[$key]->getFechaEstreno(); ?>">
	                  </div>
	                  <div class="form-group">
	                    <label for="director">Director</label>
	                    <input type="text" name="director" id="director" class="form-control"  value="<?php echo $mostrar[$key]->getDirector(); ?>">
	                  </div>
	                  <div class="form-group">
	                    <label for="reparto">Reparto</label>
	                    <input type="text" name="reparto" id="reparto" class="form-control"  value="<?php echo $mostrar[$key]->getReparto(); ?>">
	                  </div>
	                   <div class="form-group">
	                    <label for="sinopsis">Sinopsis</label>
	                    <input type="text" name="sinopsis" id="sinopsis" class="form-control"  value="<?php echo $mostrar[$key]->getSinopsis(); ?>">
	                  </div>
	                  <div class="form-group">
	                  	 <label for="trailer">Trailer</label>
	                    <input type="text" name="trailer" id="trailer" class="form-control"  value="<?php echo $mostrar[$key]->getTrailer(); ?>">
	                  </div>            
	                  <input name="modificar" id="modificar" class="btn btn-primary  login-btn mb-4" type="submit" value="Modificar Pelicula">
	                </form>
	               </div>
	                <?php
	                     } 
